		...
		if (!is_admin()) add_action( 'wp_print_styles', 'gab_shortcodecss' );
			if (!function_exists('gab_shortcodecss')) {
				function gab_shortcodecss() {
					wp_enqueue_style('gabfire_shortcodes', get_template_directory_uri() .'/framework/functions/shortcodes.css');
				}
		}
		require_once (GABFIRE_FRAMEWORK_PATH . '/functions/shortcodes.php'); 	
	}
 ...
 ******************************************************************** */
	function gab_title() {
		...
		if ( $paged >= 2 || $page >= 2 )
			echo ' | ' . sprintf( __('%s', 'source'), max( $paged, $page ) );	
	}
 ...
 ******************************************************************** */
	...
	function gab_postcomment() { /* We first create a function to get the post permalink with read more anchor */
		...
		//MK Comment changed
		comments_popup_link(__('Comment','source'), __('1 Comment','source'), __('% Comments','source'));
		echo '</span>';
	}	
...
/* ********************
 * Add Twitter, Google+ and Facebook fields
 * into WordPress admin -> user field.
 ******************************************************************** */
	function add_contactmethod( $user_contactmethods ) {
	   return array_merge( $user_contactmethods, array( 'twitter' => 'Twitter Username', 'facebook' => 'Facebook URL', 'googleplus' => 'Google+ URL' ) );
	}
				...
				<div class="col left">
					...
					<form method="post" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>" id="registerform" name="registerform">
						...
						<p class="submit">
							...
							<input type="hidden" name="redirect_to" value="<?php echo home_url(); ?>/wp-admin/profile.php" />
